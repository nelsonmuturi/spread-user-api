...
public class RestApiErrorHandler {
  ...
  public ResponseEntity<Error> handleAuthenticationException(
    ...
    logger.info("exception.getCause() = " + ex.getCause());
    String errorMsg = "";
  ...
  @ExceptionHandler(DependentResourceFoundException.class)
  public ResponseEntity<Error> handleDependentResourceFoundException(HttpServletRequest request,
                                                                     DependentResourceFoundException ex, Locale locale) {
    Error error = ErrorUtils
            .createError(
                    String.format("%s %s", ErrorCode.DEPENDENT_RESOURCE_FOUND.getErrMsgKey(), ex.getMessage()),
                    ex.getErrorCode(),
                    HttpStatus.NOT_ACCEPTABLE.value()).setUrl(request.getRequestURL().toString())
            .setReqMethod(request.getMethod())
            .setTimestamp(Instant.now());
